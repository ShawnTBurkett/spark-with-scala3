...
object Foo {
  val defaultValue = "instantiation_value"
  val nonDefaultValue = s"non_$defaultValue"
  var _foo: String = defaultValue
  def foo: String = _foo
  def foo_=(newFoo: String): Unit = _foo = newFoo
  def setNonDefaultValue(): Unit = foo = nonDefaultValue
}
